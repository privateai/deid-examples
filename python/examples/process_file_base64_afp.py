# Example script to illustrate how to make async API calls to the Private AI API
...
def submit_job(file_request: dict) -> str:
    ...
    if not job_id:
        raise ValueError(f"No job ID received in response: {response.json()}")
...
def download_file(job_id: str, output_file_name: str) -> None:
    ...
    with requests.get(download_url, headers={"user-id": USER_ID}) as response:
        response.raise_for_status()
...
def process_file(file_name: str) -> None:
    ...
    while True:
        ...
        if state in ["completed", "downloaded"]:
            ...
            print(f"Deidentification of {file_name} failed with state: {state}")
            raise ValueError(f"Error description: {description}")
        else:
